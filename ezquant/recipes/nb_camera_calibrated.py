...
class NBCameraCalibratedRecipe(RecipeBase):
    def describe(self) -> RecipeDescription:
        return RecipeDescription("nb_camera_calibrated", "0.1.0", "Optional notebook-only recipe placeholder", ["pixel_size_um"])
    ...
    def build_outputs(self, raw_results, context) -> AnalysisResult:
        return AnalysisResult(tables=[AnalysisTable(name="nb_camera", rows=[])])
